...
class RandomColor(object):
    ...
    def __init__(self, multiplier_range=(0.9, 1.1), brightness_mult_range=(0.9, 1.1), random_state=np.random,
                 dataset='KITTI'):
...
class Resize(object):
    ...
    def __init__(self, size=224, interpolation='bilinear'):
        self.interpolation = interpolation
        self.size = size

    def __call__(self, img, img_type='rgb'):
        assert isinstance(self.size, int) or isinstance(self.size, float) or \
               (isinstance(self.size, collections.Iterable) and len(self.size) == 2)
        if img_type == 'rgb':
            return scipy.misc.imresize(img, self.size, self.interpolation)
        elif img_type == 'depth':
            if img.ndim == 2:
                img = scipy.misc.imresize(img, self.size, self.interpolation, 'F')
            elif img.ndim == 3:
                img = scipy.misc.imresize(img[:, :, 0], self.size, self.interpolation, 'F')
