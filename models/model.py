...
class DepthBLIP(nn.Module):
    def __init__(self, model, txt_processors, args, flag=1):
        ...
        if self.method == 'first':
            ...
            self.register_buffer("bin_tensor", bin_tensor)
            # text_f: (768, bins)
    ...
    def forward(self, input_rgb_img, i):
        ...
        # trans type
        input_rgb_img = input_rgb_img.to(self.bin_tensor.device, dtype=self.dtype)
        if self.args.auto_bins:
        ...
        if self.method == 'first':
            ...
            img_f = img_f.to(device=self.device, dtype=self.dtype)
        ...
        # if chose method 'second' to finish depth prediction
            ...
            if self.args.dataset == 'KITTI':
                # outdoor: Q-Former 768-dim → (1, 16, 48)
                dim = (output_depth_list.shape[0], output_depth_list.shape[1], 1, 16, 48)
            else:
                # indoor (NYU, SUN3D, B3DO): Q-Former 768-dim → (3, 16, 16)
        ...
        output_depth = output_depth * bin_tensor.view(1, -1, 1, 1).to(output_depth.device)
